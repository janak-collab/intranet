...
// Medication-specific initialization
    ...
    medicationItems.forEach(item => {
        ...
        if (mainRadio && details) {
            mainRadio.addEventListener('change', function() {
                handleMedicationSelection(this);
            });
            
            // Initialize detail inputs
            const detailInputs = details.querySelectorAll('input[type="radio"]');
            detailInputs.forEach(input => {
                input.disabled = true;
                input.addEventListener('change', validateMedicationDetails);
            });
        }
    });

    // Handle "No medication" selections
    document.querySelectorAll('.clear-selection input[type="radio"]').forEach(radio => {
        radio.addEventListener('change', function() {
            if (this.checked) {
                clearCategorySelections(this.name);
            }
        });
    });
}

function handleMedicationSelection(radio) {
    const categoryInputs = document.querySelectorAll(`input[name="${radio.name}"]`);
    ...
    categoryInputs.forEach(input => {
        ...
        if (details && detailInputs) {
            if (input.checked && input.value !== '') {
                details.style.display = 'flex'; // Changed from .active class
                detailInputs.forEach(input => input.disabled = false);
            } else {
                details.style.display = 'none'; // Changed from .active class
                detailInputs.forEach(input => {
                    input.disabled = true;
                    input.checked = false;
                });
...
function clearCategorySelections(categoryName) {
    const categoryItems = document.querySelectorAll(`.medication-item input[name="${categoryName}"]`);
    
    categoryItems.forEach(radio => {
        const item = radio.closest('.medication-item');
        const details = item?.querySelector('.medication-details');
        
        if (details) {
            details.style.display = 'none'; // Changed from .active class
            details.querySelectorAll('input[type="radio"]').forEach(input => {
                input.disabled = true;
                input.checked = false;
            });
        }
...
function validateMedicationDetails() {
    ...
    const mainRadio = medicationItem?.querySelector('input[type="radio"][data-medication]');
    
    if (!mainRadio?.checked) return;

    const medKey = mainRadio.dataset.medication;
    const doseSelected = document.querySelector(`input[name="${medKey}_dose"]:checked`);
    const frequencySelected = document.querySelector(`input[name="${medKey}_frequency"]:checked`);

    if (!doseSelected || !frequencySelected) {
        showMedicationError(medicationItem, 'Please select both dose and frequency');
    } else {
        clearErrorMessages();
    }
}

function showMedicationError(item, message) {
    clearErrorMessages();
...
// Form validation for medications
function validateMedicationSelections(formData) {
    const errors = [];
    
    // Validate each medication category
    ['NSAID', 'mrelaxer', 'nerve_agent'].forEach(category => {
        const selectedMed = formData.get(category);
        if (selectedMed) {
            const doseSelected = formData.get(`${selectedMed}_dose`);
            const frequencySelected = formData.get(`${selectedMed}_frequency`);
            
            if (!doseSelected || !frequencySelected) {
                errors.push(`Please complete dose and frequency for selected medication`);
            }
        }
    });
...
function initializeFormValidation() {
    ...
    if (firstName && lastName) {
        ...
        [firstName, lastName].forEach(input => {
            const errorDiv = document.createElement('div');
            errorDiv.className = 'field-error';
            errorDiv.textContent = `Please enter your ${input.name.replace('_', ' ')}`;
            input.parentNode.appendChild(errorDiv);
            errorDiv.style.display = 'none';
...
function handleQutenzaSelection() {
    ...
    const treatmentDetails = document.getElementById('qutenza-details');
    if (!qutenzaCheckbox || !treatmentDetails) return;

    // Debug initial state
    console.log('Initial Qutenza checkbox state:', qutenzaCheckbox.checked);
    
    function updateSideInputs(enabled) {
        const sideInputs = document.querySelectorAll('input[name="qutenza_side"]');
        sideInputs.forEach(input => {
            input.disabled = !enabled;
            console.log(`Side input ${input.value} disabled:`, input.disabled);
        });
    }

    function updateAreaInputs(enabled) {
        ...
        areaInputs.forEach(input => {
            ...
            console.log(`Area input ${input.value} disabled:`, input.disabled);
        });
    }

    // Handle main Qutenza checkbox changes
    qutenzaCheckbox.addEventListener('change', function() {
        console.log('Qutenza checkbox changed:', this.checked);
        
        treatmentDetails.style.display = this.checked ? 'block' : 'none';
        
        if (this.checked) {
            updateSideInputs(true);  // Enable side inputs
            updateAreaInputs(false); // Disable area inputs initially
        } else {
            updateSideInputs(false); // Disable side inputs
            updateAreaInputs(false); // Disable area inputs
            
            // Clear all selections
            document.querySelectorAll('input[name="qutenza_side"], input[name="qutenza_area"]')
                .forEach(input => input.checked = false);
        }
    });

    // Handle side selection changes
    document.querySelectorAll('input[name="qutenza_side"]').forEach(sideInput => {
        sideInput.addEventListener('change', function() {
            console.log('Side selection changed:', this.value, 'Checked:', this.checked);
            
            if (this.checked && !this.disabled) {
                console.log('Enabling area inputs');
                updateAreaInputs(true);
            }
        });
    });

    // Set initial state
    if (qutenzaCheckbox.checked) {
        treatmentDetails.style.display = 'block';
        updateSideInputs(true);
        
        // If a side is already selected, enable area inputs
        const selectedSide = document.querySelector('input[name="qutenza_side"]:checked');
        if (selectedSide) {
            updateAreaInputs(true);
        } else {
            updateAreaInputs(false);
        }
    } else {
        treatmentDetails.style.display = 'none';
        updateSideInputs(false);
        updateAreaInputs(false);
    }
}
